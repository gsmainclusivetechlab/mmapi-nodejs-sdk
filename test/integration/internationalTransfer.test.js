...
const createInternationalTransaction = async (
  ...
  try {
     ...
     */
    const request =
      new mmapi.internationalTransfer.createInternationalTransaction();
...
const createReversal = async (
  ...
  try {
     ...
     * Construct a request object and set desired parameters
     */
    const request = new mmapi.internationalTransfer.createReversal(
      originalTransactionReference
    );
...
const viewAccountBalance = async (debug = false) => {
  try {
     ...
     * Construct a request object and set desired parameters
     */
    const request = new mmapi.internationalTransfer.viewAccountBalance({
      walletid: "1",
    });
...
const viewAccountTransactions = async (debug = false) => {
  try {
     ...
     * Construct a request object and set desired parameters
     */
    const request = new mmapi.internationalTransfer.viewAccountTransactions({
      accountid: "2999",
    });
    ...
    if (debug) {
      ...
      console.log("Response Data: ", JSON.stringify(response.data, null, 4));
      console.log(
        "Response X-Records-Available-Count",
        response.headers["x-records-available-count"]
      );
      console.log(
        "Response X-Records-Returned-Count",
        response.headers["x-records-returned-count"]
      );
    }
...
const viewQuotation = async (quotationReference, debug = false) => {
  try {
     ...
     * Construct a request object and set desired parameters
     */
    const request = new mmapi.internationalTransfer.viewQuotation(
      quotationReference
    );
...
const viewRequestState = async (serverCorrelationId, debug = false) => {
  try {
     ...
     * Construct a request object and set desired parameters
     */
    const request = new mmapi.internationalTransfer.viewRequestState(
      serverCorrelationId
    );
...
const viewResponse = async (clientCorrelationId, debug = false) => {
  try {
     ...
     * Construct a request object and set desired parameters
     */
    const request = new mmapi.internationalTransfer.viewResponse(
      clientCorrelationId
    );
...
const viewTransaction = async (transactionReference, debug = false) => {
  try {
     ...
     * Construct a request object and set desired parameters
     */
    const request = new mmapi.internationalTransfer.viewTransaction(
...
describe("International Transfers", () => {
  describe("Perform an International Transfer", () => {
    ...
    describe("POST Perform an International Transfer", () => {
      it("should return the request state object with status 202 to indicate that the request is pending", async () => {
        const response = await createInternationalTransaction(
          "REF-1636533162268",
          true,
          false
        );
  ...
  describe("Check for API Provider Service Availability", () => {
    describe("GET Check for Service Availability", () => {
      it("should return the heartbeat object with status 200 to indicate the status available, unavailable or degraded", async () => {
        ...
        expect(response.data.serviceStatus).toMatch(
