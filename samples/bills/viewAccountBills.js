 ...
 */
const viewAccountBills = async (useCase, accountIdentifiers = buildAccountIdentifiers(), query = buildRequestQuery(), debug = false) => {
  try {
    /**
     * Construct a request object and set desired parameters
     */
    const request = new mmapi[useCase].viewAccountBills(accountIdentifiers);
...
if (require.main === module) {
  ...
  (async () => {
    try {
      await viewAccountBills('<<REPLACE-WITH-USE-CASE>>', undefined, undefined, true);
    } catch (err) {
    }
  })();
}
