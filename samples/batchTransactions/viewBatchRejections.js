 ...
 */
const viewBatchRejections = async (useCase, batchId, query = buildRequestQuery(), debug = false) => {
  try {
    /**
     * Construct a request object and set desired parameters
     */
    const request = new mmapi[useCase].viewBatchRejections(batchId);
...
if (require.main === module) {
  ...
  (async () => {
    try {
      await viewBatchRejections('<<REPLACE-WITH-USE-CASE>>', '<<REPLACE-WITH-BATCH-ID>>', undefined, true);
    } catch (err) {
    }
  })();
}
